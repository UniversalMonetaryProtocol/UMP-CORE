...
const char * const BITCOIN_CONF_FILENAME = "universalmonetaryprotocol.conf";
const char * const BITCOIN_PID_FILENAME = "universalmonetaryprotocol.pid";
...
#else
    const char* pszModule = "universalmonetaryprotocol";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "UniversalMonetaryProtocol";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/UniversalMonetaryProtocol";
#else
    // Unix
    return pathRet / ".universalmonetaryprotocol";
#endif
#endif
}
